...
11. 检查范围、仓库、实现状态和“当前/已批准”断言是否记录 `evidence_state`、`basis` 和
    `evidence_refs`；用户确认只直接支持意图或选择，不自动证明外部事实。
12. 检查 `confidence` 是否默认 `unassessed`；导航收据的 `high`、历史批准、目录存在或
    Contract 未列路径不得被升级为明确排除、当前事实或读取许可。
