...
如果修改了 `10_reality`，除通用 checker 外还必须：

```bash
./scripts/maglev-python .agents/skills/crystallization/references/scripts/crystallization_check.py <reality-module>
```

- 从 candidate commit 创建独立 Validator worktree；
- 验证完整 Reality tree 和 base → candidate diff；
- 生成绑定 Projection digest 的 Validation Result；
- 调用共享 Reality Admission；
- 只有 `accepted` 或 `no_change` 才能声明 Reality 变化已准入。

通用 checker 不能替代 Projection Validation 或 Admission。
