...
- [ ] 存在唯一默认 Coordinator。
- [ ] 通用 `routing_responsibility` 已映射为 Maglev `coordinator`，且 leader、manifest 和 Agent 角色一致。
...
- [ ] 共享产物带有 commit、声明 ref 和 `receiver_sync` 证据。
- [ ] Coordinator 消费记录包含 `decision_fingerprint`、调度预算和 `ready_to_close` 停止条件。
- [ ] 人类路由消费 `shared_contract.human_role_routing`，支持多角色、第一顺位和空 `human_targets`。
- [ ] 审查包含 Review Packet、单一主修复角色和独立复审。
