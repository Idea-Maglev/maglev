...
- 通用方法的 `routing_responsibility` 在 Maglev 模板中映射为唯一的 `coordinator`；这是 Maglev Adapter 的样本实现，不回写为通用小队的固定角色名。
- 共享产物准入映射为 Handoff 的 Git `commit`、声明的 `ref`、`push_result` 和接收方 `receiver_sync`；缺少同步回执时保持 `awaiting_external` 或 `blocked`。
- 有限调度映射为 Coordinator 消费记录中的 `decision`、`decision_fingerprint`、`budget` 和 `ready_to_close`，不把“已派发”解释为已完成。
- 人类责任路由由 `shared_contract.human_role_routing` 配置和 Coordinator 消费；成员描述、顺序、多角色和空 `human_targets` 都由 Adapter 负责保存和验证。
- 产物审查映射为生产者 skill 只读追溯、Review Packet、单一主修复角色和独立复审；Maglev Adapter 不要求生产者 skill 增加接口。
