...
所有聊天和未声明受众的文件默认面向人类。需要人类判断的非代码产物必须提供 Markdown 文档格式（Markdown）审阅面；机器来源、来源摘要和 Markdown 映射的路径关系由人类可读输出检查器确定性验证。该检查器不判断正文是否满足读者目标，也不要求固定标题、章节、篇幅或页面形状。

共享规则：`.agents/skills/_internal/human-readable-output/contract.md`。
