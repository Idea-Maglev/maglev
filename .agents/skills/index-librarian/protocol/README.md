...
- 所有索引产物由 `track_scan.py` 独占写入，AI 不得手工编辑。
- 旧 registry 中的 `map_output` 暂时兼容读取但不再执行；人读地图统一由 `maglev-map-maker` 生成 `docs/ATLAS.md`。
...
运行顺序是 scan → verify。`task_navigate.py` 只负责返回来源选择收据，不能代替 scan 或 verify。
