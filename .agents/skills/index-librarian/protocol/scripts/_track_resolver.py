...
Used by: track_scan.py / track_verify.py
...
def _validate_track(track: Any, *, log_prefix: str = "[track-resolver]") -> bool:
    ...
    for field in ("output", "map_output"):
        if field in track and not isinstance(track[field], str):
            ...
            return False
    if track.get("map_output"):
        print(
            f"{log_prefix} warn: track {track['id']!r} uses deprecated "
            "'map_output'; Markdown maps are now generated by maglev-map-maker"
        )
