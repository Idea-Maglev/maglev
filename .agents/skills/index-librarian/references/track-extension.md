...
| `repo-entry` | 仓库根目录 | `repo-entry.yaml` |
...
| 出现 `map_output` 弃用提示 | registry 仍保留旧字段 | 删除该字段；人读地图由 `maglev-map-maker` 生成 |
| 单文件叶目录出现重复导航 | 目录和其唯一文件都各自有入口 | 仅对明确模式配置 `collapse_single_file_dirs`，让父级直接记录叶文件 |

## 4. 不要做的事

- ❌ 不要手编 `INDEX.md` / `repo-entry.yaml` / `code-tree.yaml` —— 产物由脚本独占写权。
