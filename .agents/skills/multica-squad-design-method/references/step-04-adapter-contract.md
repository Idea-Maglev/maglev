...
description: 定义具体项目或第三方承载如何承接通用小队设计
...
把通用小队设计交给具体项目或第三方承载落地，并防止通用方法假设不存在的运行环境。
...
2. 填写 Adapter Contract：
   ...
   - 命令面：校验、测试、同步、发布命令。
   - 身份面：对象归属如何判断，是否依赖 marker、lock、远端 ID 或其他真实身份锚点。
   - 权限面：谁可读、谁可写、谁可批准。
   - 写入门禁：外部状态变更前需要什么批准、版本证据和回执。
   - 验证面：哪些证据只能证明 L2，哪些第三方回执才能证明 L3。
3. 检查 Adapter 不得偷换声明：
   - 不能把 L1 设计称为 L2 模板验证。
