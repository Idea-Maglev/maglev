...
评估 Maglev 时，应把“当前事实”“可复算的静态检查”和“仍需要运行或人工确认的结论”分开。Reality README 给出阅读范围和限制；各能力域的 verification 页面给出对应缺口，不要只看 capability 概览。
...
- 看本页机制在主链路中何时被消费：[核心工作流](lifecycle-and-governance.md)
- 看索引背后的能力登记体系：[能力登记册](capability-landscape.md)
- 看整体架构中的位置：[架构总览](architecture-overview.md)
...
Wiki authoring 在写作或改稿前读取当前 Reality、正式指南和任务边界，形成简短起点简报，再进入 Plan/Challenge/Approval 约束下的正文写作。该过程不拥有事实，也不替代页面审查；评估证据仍以 Reality 和正式指南为准。
...
- 在日常流中执行沉淀检查：[日常工作流](../developer/session-workflow.md)
- 协作流程与 Agent 使用边界：[从请求到结晶的日常协作](../developer/session-workflow.md)。
- 从首次交付走查理解知识如何随交付沉淀：[首次交付走查](../developer/first-success.md)
- 老项目接入时如何重建分层知识：[存量项目接入](../developer/brownfield-adoption.md)
