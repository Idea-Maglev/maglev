...
> 本页覆盖 Maglev 能力体系的登记与治理事实：主链路与技能目录（`.agents/private-catalog.yaml`）、竞品观测与技能侦察（`competitive-registry.yaml`、skill-scout）、文档源治理（`documentation-governance.json`）与运营手册出口（`docs/guides/`）。数据时点为 2026-09-03 仓库现状。

## 评估者先读：接入三问

| 问题 | 回答 |
|------|------|
| 接在哪一层 | 登记与治理层：为技能运行时提供"现役能力清单、关系图与受管表面"，不进入代码执行层本身（见 specs/10_reality/skill-runtime/capability/overview.md） |
| 与现有体系冲突吗 | 不与外部能力竞争，而是给它们设准入闸：外部 skill 必须经插槽从 lock 解析、选择后才生效，禁止自动触发绕过需求/方案/Slot 纪律（见 specs/10_reality/skill-runtime/capability/overview.md §2.2） |
| 要不要一次铺满 | 不需要。登记表按治理价值渐进登记——"纯包装层、临时入口、无独立治理价值的对象，不必强行登记"（见 .agents/private-catalog.yaml 头注释） |
...
| 登记表 | 位置 | 登记什么 | 谁消费 |
|--------|------|----------|--------|
| 私域能力清单 | `.agents/private-catalog.yaml` | 现役治理对象及其关系；数量以当前清单为准 | 主流程编排、skill-scout / skill-squadron 巡逻 |
| 竞品注册表 | `specs/10_reality/capability-evolution/evidence/competitive-registry.yaml` | 观察对象清单 + insight 状态 + 维度升级记录 | evolution-observatory 观测循环 |
| 文档治理注册表 | `specs/_meta/documentation-governance.json` | canonical owner、派生投影、消费者、历史边界 | 受管表面生成器与漂移检查器 |
| 运营手册 | `docs/guides/`（五段结构） | 面向使用者的操作知识（非登记表，是治理后的出口） | 使用者按 README 角色分流阅读 |

## 能力清单：主链路、横切能力与治理对象

**问题**：新会话抓错主线、不知道哪些能力现役、能力之间什么关系——这些问题的根源是能力清单散落在目录里靠人记忆。

**抓手**：`.agents/private-catalog.yaml` 作为技能注册的单一权威，加上由治理注册表生成的主链路定义。
...
| 7 | `crystallization` | 结晶回写 `specs/10_reality` |
| 横切 | `knowledge-check` | 知识沉淀检查，按需触发不占链路位置 |

（见 specs/10_reality/skill-runtime/capability/overview.md §2.2；执行链为 `entry-router → spec-designer → code-execution-slot → selected entry skill | agent-native → integrated-validator`）
...
（见 specs/10_reality/skill-runtime/capability/overview.md §2.2）

### 私域能力清单的登记与生命周期

`.agents/private-catalog.yaml` 是"治理对象清单"，不是 `.agents/skills/` 与 `.agents/workflows/` 的机械镜像，也不是历史日志。登记与生命周期规则：
...
当前清单登记的对象以 `.agents/private-catalog.yaml` 和生成的 skills index 为准；页面不复制会随版本变化的数量快照。登记对象覆盖主链路、横切治理、索引与地图、能力进化和分析等类别。
...
| 阶段 | 内容 | 约束 |
|------|------|------|
| Phase 0 | 读 `specs/10_reality/positioning.md` 锚定开局 | 每轮开始前必须执行 |
...
触发方式是 Creator 主动触发（AI 不自动启动研究）；研究报告归档至 `docs/thinking/10_critique/`，commit 前缀 `research(observatory)`。（见 specs/10_reality/capability-evolution/implementation/evolution-cycle.md §1）
...
（见 specs/10_reality/capability-evolution/implementation/evolution-cycle.md §1）
...
（见 specs/10_reality/capability-evolution/evidence/competitive-registry.yaml 头注释与 specs/10_reality/capability-evolution/implementation/evolution-cycle.md §1）

## 技能侦察：skill-scout 双模式

**问题**：想引入外部能力但不想从头设计；已登记对象怎么持续优化而不腐化？

**抓手**：skill-scout 的 Scout（引入）与 Patrol（巡逻）两种模式，登记落点统一为 `.agents/private-catalog.yaml`。
...
Scout 模式把外部 skill/workflow 经私域化改造后生成并登记为私域能力对象；Patrol 模式对已登记对象做 diff 巡逻并产出结构化 PatrolReport（YAML：`generated_at` / `summary` / `opportunities`（含 `value_score`）/ `status`）。（见 specs/10_reality/capability-evolution/capability/overview.md §2 与 specs/10_reality/capability-evolution/implementation/evolution-cycle.md §1）
...
**抓手**：`docs/guides/` 五段结构 + README 按角色分流。
...
`docs/guides/README.md` 以"如果你只想知道先看什么"开头，按五类角色分流到具体篇目：第一次接触 Maglev 的人、维护 Maglev 本身的维护者、关心"为什么这样设计"的读者、评估老项目接入路径的读者、公司私域环境的使用者。（见 specs/10_reality/operations-docs-system/capability/overview.md §1 与 docs/guides/README.md）
...
| 允许外部 skill 绕过主链路自动生效 | 必须经插槽从 lock 解析、选择后才生效（见 specs/10_reality/skill-runtime/capability/overview.md §2.2） |

## 来源

- specs/10_reality/skill-runtime/capability/overview.md —— 技能注册、优先级协议与插槽边界
- .agents/private-catalog.yaml —— 私域能力清单单一权威（登记字段与生命周期规则）
- specs/10_reality/capability-evolution/implementation/evolution-cycle.md —— 观测 6 Phase、insight 状态机与侦察双模式机制
- specs/10_reality/capability-evolution/capability/overview.md —— 能力进化域对象清单、触发入口与边界
- specs/10_reality/capability-evolution/evidence/competitive-registry.yaml —— 竞品注册表本体与 Insight Schema v2
- specs/10_reality/governance-quality/implementation/documentation-governance.md —— 文档源治理与漂移检查
- specs/10_reality/operations-docs-system/capability/overview.md —— 运营文档知识能力
- docs/guides/README.md —— 运营手册五段结构与角色分流
