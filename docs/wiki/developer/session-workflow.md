...
接入 Maglev 后，每个需求都走同一个循环：**发起需求 → 推进方案与实施 → 验证 → 收尾**。三个角色分工：VO（Value Owner）定义意图，TP（Tech Pilot）推进技术实施，XG（Experience Guardian）负责体验验收（角色职责与协作出处：[协作手册](../../../source operation guides/20_operations/collaboration_playbook.md)）。本页以"用户登录"功能为例，把一个循环完整走一遍。
...
需求文件写完不等于收敛。交给 TP 推进方案前，VO 按 Ready Gate 做 6 条自检，全部满足才算收敛、才进方案设计（出处：[项目术语表](../../../internal Reality/glossary.md)）：
...
看板只做观测、不驱动流程：它不推进需求、不做工时/绩效统计、不做 commit 级追踪。看到某需求停在某个阶段，仍由对应角色按本页循环去推进，而不是等看板动它（出处：[协作生命周期能力概览](../../../internal Reality/collaboration-lifecycle/capability/overview.md)）。
...
- AI 发疯（代码完全跑不通、逻辑胡编乱造）时熔断：停止 Prompting，不要试图"说服"AI；手动接管并回退到上一个稳定版本；分析原因（通常是 Context 太长或 Spec 描述矛盾）；VO 简化 Spec，或把大任务拆解成小任务（熔断步骤出处：[协作手册](../../../source operation guides/20_operations/collaboration_playbook.md)）
...
各入口的定义与适用边界见[Maglev 入口说明](../../../source operation guides/20_operations/maglev_entrypoints.md)。
