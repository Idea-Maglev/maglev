...
- `.maglev/extensions.sources.yaml`
  - 当前项目的扩展索引源配置
  - 第一次初始化时会默认写入官方扩展源 `maglev-official`
  - 初始化会尽量自动选通 SSH / HTTP(S) 中当前环境可访问的那一条；若没选通，也不会阻断 init
...
它代表当前项目已经默认接入官方扩展源。这样你第一次使用扩展时，不需要先自己去找 source 地址。
...
3. 如果你准备使用扩展，先执行 `npx @idea-maglev/maglev-extension-cli sources list --json` 看看默认官方源是否已经存在
4. 再通过 AI 工作流了解当前项目状态，例如 `/standup`
5. 需要更新时，再使用 `update`
...
### Q4：执行 `init` 后为什么会多出 `.maglev/extensions.sources.yaml`？

这是当前默认行为。第一次初始化时，Maglev 会自动写入官方扩展源，方便你后续直接搜索和管理扩展；初始化还会 best-effort 尝试当前环境可访问的协议，但即使探测失败，也只会回退写 canonical 官方源，不会把 init 判成失败。

### Q5：我已经初始化过，还能再执行 `init` 吗？
