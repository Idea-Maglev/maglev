...
### 第四步：如果你准备使用扩展，顺手确认扩展源是否已经就绪

对较早接入 Maglev 的项目，`.maglev/extensions.sources.yaml` 可能还不存在。这时可以先执行：

```bash
npx @idea-maglev/maglev-extension-cli sources list --json
```

如果输出里已经有 `maglev-official`，说明当前项目已经能直接搜索官方扩展。

如果 source 还不存在，再补一次默认官方源：
...
### 6. 如果这是较早初始化的项目，扩展源是否已经补齐

如果这个项目是在“默认官方扩展源”上线之前接入的，建议额外确认：

- `.maglev/extensions.sources.yaml` 是否存在
- `npx @idea-maglev/maglev-extension-cli sources list --json` 是否能看到 `maglev-official`

这样你后续再用扩展 CLI 时，就不会在第一次搜索扩展时才发现 source 缺失。
...
如果这里已经能看到 `maglev-official`，说明当前项目已经具备搜索官方扩展的前置条件。
...
### Q5：老项目里没有 `.maglev/extensions.sources.yaml` 怎么办？
...
> 已接入的项目先用 `update --dry-run` 预览，再执行 `update`，并在更新后检查 `.maglev/sync_state.json`、AI 上下文结果、子仓库管理状态，以及扩展源是否已经就绪。
