...
- 返回 `sources_missing`
- 或者当前项目里根本没有 `.maglev/extensions.sources.yaml`

原因：

- 这是一个已经接入 Maglev 的项目，但还没配置任何扩展 source
- 项目可能是在“默认官方扩展源”上线之前初始化的
- 你在新项目文档里看到了默认行为，但当前项目还没补到这一步
...
- 是否存在 `.maglev/extensions.sources.yaml`
- `sources list` 结果里是否已经有 `maglev-official`

怎么处理：

- 如果 source 已经存在，优先检查是不是在错误目录执行
- 如果 source 不存在，补上默认官方源：
...
- `sources add` 会在缺失时创建 `.maglev/extensions.sources.yaml`
- 补完后再搜一次，就能验证问题是不是单纯由 source 缺失引起
- 如果你配置了多个 enabled source，当前 CLI 会跳过单个失效 source 并继续尝试后续健康 source；这时 JSON 结果里会带 warning，提醒你哪个 source 被跳过

## 6. 问题 5：Shell 入口无法下载安装器
...
## 8. 问题 7：本地离线源缺文件或 manifest 解析失败
...
## 10. 问题 9：更新后根目录仍然出现 `install.sh` 或 `maglev_installer.py`
...
5. 如果你在排扩展问题，先执行 `npx @idea-maglev/maglev-extension-cli sources list --json`
6. 执行 `track_verify --track-id skills` 和 `track_verify --track-id specs`
7. 再直接调用安装器做一次 `--dry-run`
8. 最后再检查发行物和镜像是否一致
