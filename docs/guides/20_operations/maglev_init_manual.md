...
如果当前项目还没有扩展 source 配置，`init` 还会自动写入默认官方扩展源，避免你第一次使用扩展时先手工补 `.maglev/extensions.sources.yaml`。

初始化会优先尝试把这个默认官方源写成你当前环境可访问的协议（SSH 或 HTTP(S)）。如果自动探测没成功，`init` 仍会继续完成，并回退写入 canonical 官方源；这不会被当成初始化失败。
...
- 这些目录用于承载 Maglev 的规范、文档、验证和本地状态
...
- `.maglev/config.json`
- `.maglev/extensions.sources.yaml`
...
- `extensions.sources.yaml`
  - 记录当前项目可用的扩展索引源
  - 如果文件原本不存在，`init` 会默认写入官方源 `maglev-official`
  - `init` 会先做一次 best-effort 协议探测，优先落盘当前环境可访问的官方 URL
  - 如果自动探测失败，会回退到 canonical 官方值 `https://github.com/Idea-Maglev/maglev.git`，`ref: master`
- `sync_state.json`
  - 记录当前版本、同步时间和文件基线哈希
...
sources:
  - id: maglev-official
    name: Maglev Official Extensions
    type: git
    url: https://github.com/Idea-Maglev/maglev.git
    ref: master
    enabled: true
```
...
2. 是否存在 `.maglev/extensions.sources.yaml`
3. 是否存在 `.maglev/sync_state.json`
4. 是否生成了 `.agents/`
5. 是否生成了 `specs/`、`docs/`、`tests/`
6. 是否存在并可执行 `scripts/maglev-python`
7. `./scripts/maglev-python --doctor` 是否成功
8. `npx @idea-maglev/maglev-extension-cli sources list --json` 是否能看到 `maglev-official`
9. `track_verify --track-id skills` 和 `track_verify --track-id specs` 是否返回 `ok`
10. 根目录是否没有出现：
   - `install.sh`
   - `maglev_installer.py`

第 10 点尤其重要，因为当前分发规则已经收口，这两个文件不应再作为目标项目根目录资产下发。

如果你接下来就准备使用扩展，建议马上执行：
...
前者用来确认默认官方源已经就绪，后者用来确认当前项目已经可以直接发现扩展。
