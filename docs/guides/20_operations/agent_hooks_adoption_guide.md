...
| 会话结束 | `Stop` | `Stop` | 只记录结束事实；汇总在用户主动导出时生成 |
...
Stop 最适合记录“结束 hook 被触发”这一事实。它处于会话关键路径，不应扫描既有 trace、生成 session summary 或承载重型验证。

需要检查验证、policy 或会话统计时，由导出器读取原始事件后按需汇总。这样 trace 越长，Stop 的同步工作量也不会随之增长。
...
每个 session 的 JSONL 是原始事实源。不要在 Hook 内生成汇总文件；需要会话或跨项目统计时，再由 exporter 读取 events 生成快照和报告。
...
**经验**：先做“事件流 + 按需导出”，不要一开始就追求复杂可视化或在同步 Hook 内生成会话摘要。只要数据结构稳定，后面接 dashboard 很容易。
